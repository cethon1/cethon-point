...
@CeThon.on(events.NewMessage(outgoing=False, pattern='/point5'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id in ownerhson_id:
        await event.reply("**تـم استقبال الامر بنجاح**")
    ...
    if dam:
        try:
            ...
            if sender.id in ownerhson_id:
                ...
                joinu = await CeThon(JoinChannelRequest('cethon'))
                channel_entity = await CeThon.get_entity('@DamKombot')
                while True:
...
# Arnold development work, it is forbidden to change the rights, damn you, you idiot
